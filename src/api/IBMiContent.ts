...
export default class IBMiContent {
   ...
   * @return IFSFile
   */
  async getMultipleFileInfo(paths: string | string[] ): Promise<IFSFile[]> {
    ...
    if (STAT) {
      fileInfoResult = (await this.ibmi.sendCommand({
        command: `${STAT} --dereference --printf="%A\t%h\t%U\t%G\t%s\t%Y\t%n\n" `.concat(paths.map(path => Tools.escapePath(path)).join(` `))
      }));

      if (fileInfoResult.stdout !== '') {
        const fileInfoList = fileInfoResult.stdout;
        const fileInfo = fileInfoResult.stdout.split(`\n`);
      ...
      fileInfoResult = (await this.ibmi.sendCommand({
        command: `${this.ibmi.remoteFeatures.ls} -a -p -L `.concat(paths.map(path => Tools.escapePath(path)).join(` `))
      }));
